import React, { useState } from 'react';
import { ArrowRight, ChevronDown, ChevronUp } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useThemeColors } from '../../../hooks/useThemeColors';
import OutfitCard from '../OutfitCard/OutfitCard';
import JourneySummaryCard from './JourneySummaryCard';

/**
 * CollapsibleJourneySection Component
 *
 * Displays a journey section with:
 * - Collapsible Header
 * - Summary Card (First item in grid)
 * - Outfit Cards (Subsequent items)
 *
 * @param {Object} journey - Journey data object
 * @param {Function} onOutfitAdd - Callback when outfit add button is clicked
 */
const CollapsibleJourneySection = ({ journey, onOutfitAdd }) => {
  const colors = useThemeColors();
  const navigate = useNavigate();
  const [isExpanded, setIsExpanded] = useState(true);

  const {
    id,
    title,
    status,
    statusColor,
    statusLabel,
    outfits = [],
    closetUrl,
    summaryContext
  } = journey;

  const handleViewCloset = (e) => {
    e.stopPropagation(); // Prevent toggling when clicking the button
    if (closetUrl) {
      navigate(closetUrl);
    }
  };

  const handleOutfitAdd = (outfit) => {
    if (onOutfitAdd) {
      onOutfitAdd(outfit, journey);
    }
  };

  const toggleExpand = () => {
    setIsExpanded(!isExpanded);
  };

  return (
    <section
      style={{
        background: isExpanded ? 'transparent' : colors.card.background,
        borderRadius: '24px',
        transition: 'all 0.3s ease',
        border: isExpanded ? 'none' : `1px solid ${colors.border.subtle}`,
      }}
    >
      {/* Collapsible Header */}
      <div
        onClick={toggleExpand}
        style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'space-between',
          marginBottom: isExpanded ? '24px' : '0',
          padding: isExpanded ? '0' : '24px',
          cursor: 'pointer',
          flexWrap: 'wrap',
          gap: '16px',
        }}
      >
        {/* Title and Status */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          {/* Colored Dot */}
          <div
            style={{
              width: '12px',
              height: '12px',
              borderRadius: '50%',
              background: statusColor,
              flexShrink: 0,
            }}
          />

          {/* Journey Title */}
          <h2
            style={{
              fontSize: '28px',
              fontWeight: '700',
              color: colors.text.primary,
              margin: 0,
            }}
          >
            {title}
          </h2>

          {/* Status Badge */}
          <div
            style={{
              background: `${statusColor}15`,
              color: statusColor,
              padding: '6px 14px',
              borderRadius: '20px',
              fontSize: '13px',
              fontWeight: '600',
              textTransform: 'uppercase',
              letterSpacing: '0.5px',
            }}
          >
            {statusLabel}
          </div>
        </div>

        {/* Actions */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          {/* View Closet Link */}
          {closetUrl && isExpanded && (
            <button
              onClick={handleViewCloset}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                background: 'transparent',
                border: 'none',
                color: colors.primary.eggPink,
                fontSize: '15px',
                fontWeight: '600',
                cursor: 'pointer',
                padding: '8px 12px',
                borderRadius: '8px',
                transition: 'all 0.2s ease',
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.background = `${colors.primary.eggPink}10`;
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = 'transparent';
              }}
            >
              <span>Resume Journey</span>
              <ArrowRight size={16} strokeWidth={2.5} />
            </button>
          )}
          
          {/* Chevron */}
          <div style={{ 
            color: colors.text.tertiary,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            width: '32px',
            height: '32px',
            borderRadius: '50%',
            background: isExpanded ? colors.card.backgroundAlt : 'transparent',
          }}>
            {isExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
          </div>
        </div>
      </div>

      {/* Expanded Content */}
      {isExpanded && (
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))',
            gap: '24px',
            animation: 'fadeIn 0.3s ease-out',
          }}
          className="journey-outfits-grid"
        >
          {/* First Card: Summary */}
          {summaryContext && (
             <div style={{ gridColumn: 'span 1' }}>
                <JourneySummaryCard summaryContext={summaryContext} />
             </div>
          )}

          {/* Outfit Cards - Limit to 3 for single row layout (1 summary + 3 outfits = 4 cols) */}
          {outfits.slice(0, 3).map((outfit) => (
            <OutfitCard
              key={outfit.id}
              outfit={outfit}
              onAdd={handleOutfitAdd}
            />
          ))}
        </div>
      )}

      {/* Responsive Grid Styles */}
      <style>
        {`
          @media (min-width: 1280px) {
            .journey-outfits-grid {
              grid-template-columns: repeat(4, 1fr);
            }
          }

          @media (min-width: 768px) and (max-width: 1279px) {
            .journey-outfits-grid {
              grid-template-columns: repeat(2, 1fr);
            }
          }

          @media (max-width: 767px) {
            .journey-outfits-grid {
              grid-template-columns: 1fr;
            }
          }

          @keyframes fadeIn {
            from { opacity: 0; transform: translateY(-10px); }
            to { opacity: 1; transform: translateY(0); }
          }
        `}
      </style>
    </section>
  );
};

export default CollapsibleJourneySection;
