...
const FindTheLookPageV2 = () => {
  ...
  const mainContentStyle = {
    display: 'grid',
    gridTemplateRows: 'auto 2.5fr 1fr 2fr 0.4fr',
    gap: '12px',
    height: '100%',
    padding: '24px 48px',
    overflow: 'hidden',
  ...
  // Look Explanation Section Styles
  ...
  const explanationTextStyle = {
    ...
    lineHeight: '1.5',
    fontWeight: '600',
    background: 'linear-gradient(135deg, #667eea 0%, #e53e3e 100%)',
        ...
        <main style={mainContentStyle}>
          ...
          {(isLoading || isRefining) ? renderLoadingState() : (
            <>
              ...
              {/* Row 3: Look Explanation (NEW in V2) */}
              {(() => {
                const explanation = selectedLook?.type === 'product' 
                  ? selectedLook.items?.[0]?.description 
                  : selectedLook?.brand;
                return explanation && (
                  <section style={{ overflow: 'hidden' }}>
...
export default FindTheLookPageV2;
